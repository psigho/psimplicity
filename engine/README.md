...
> Windows shortcut: skip all of this and double-click `START.bat` — it checks Python, asks for your key, installs dependencies, and launches the app.
...
├── config.json               # Model & pipeline configuration (wired to UI + providers)
├── requirements.txt          # Python dependencies
├── requirements-dev.txt      # pytest (offline test suite)
├── START.bat                 # Windows one-click installer/launcher
├── .env.example              # Template for .env (your keys — never committed)
│
├── modules/
│   ├── scene_parser.py       # Script → structured scenes (LLM)
│   ├── prompt_builder.py     # Scene → image generation prompt
│   ├── image_provider.py     # Image engines (Gemini/GPT-Image/DALL-E/Imagen/Seedream)
│   ├── art_director.py       # AI critique + scoring (8 dimensions)
│   ├── feedback_loop.py      # Typed errors → targeted prompt surgery
│   ├── story_bible.py        # Visual DNA consistency across scenes
│   ├── brand_bible.py        # Extract a visual identity from reference images
│   ├── costing.py            # Pre-run estimate, hard budget ceiling, rate limiter
│   ├── llm_gateway.py        # Multi-provider LLM abstraction + telemetry
│   └── orchestrator.py       # Pipeline controller (ties everything together)
│
├── style_presets/            # 12 built-in visual style configs (add your own!)
├── tests/                    # 45 offline unit tests (pytest, zero API spend)
│
└── output/                   # Generated images + reports (per run)
    └── run_ProjectName_YYYYMMDD_HHMMSS/
        ├── images/           # Every attempt + *_final.png per scene
        ├── scenes.json       # Parsed scene data (powers the redo button)
        ├── story_bible.json  # Shared visual DNA
        ├── llm_calls.jsonl   # Per-call cost/token telemetry (images included)
        └── qc_report.json    # Full QC report: scores, attempts, your 👍/👎 verdicts
```

`model_catalog.json` and `oga_models_reference.json` are **reference data only** — nothing loads them at runtime.
