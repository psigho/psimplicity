...
# ── post-grill F1.1: surface a loud warning at import time if the optional
# ── `json_repair` lib is missing. Layer 3 of extract_json() silently no-ops
# ── without it, and Gemini-style malformed-newline JSON ghost-fails Layer 1+2.
try:
    import json_repair as _jr_probe  # noqa: F401
    _HAS_JSON_REPAIR = True
except ImportError:
    _HAS_JSON_REPAIR = False
    logger.warning(
        "⚠️  Optional dep `json-repair` is NOT installed. Layer-3 JSON recovery is disabled. "
        "Install with: pip install 'json-repair>=0.30.0' — required to handle Gemini's "
        "malformed-newline quirks reliably."
    )
