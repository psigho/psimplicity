...
def create_provider(config: dict) -> ImageProvider:
    ...
    Priority: gpt_image > Seedream > Gemini API key > Service Account.
    """
    # ── GPT-Image (OpenAI) ─────────────────────────────────────────────
    # post-grill F4.3/F4.4: new provider for OpenAI's gpt-image family
    if "gpt_image" in config:
        cfg = config["gpt_image"]
        api_key = cfg.get("api_key", "")
        if not api_key:
            raise ValueError("gpt_image config is set but api_key is empty. Set OPENAI_API_KEY.")
        from .image_providers.gpt_image import GPTImageProvider
