...
class ArtDirector:
    ...
    # ── post-grill F1.2: split into STATIC system_instruction (rubric — cacheable) ──
    # ── and DYNAMIC user prompt (scene context — varies per call).                ──
    CRITIQUE_SYSTEM = """You are a professional Art Director reviewing AI-generated images for a video production pipeline.

SCORE EVERY IMAGE on these 8 dimensions (each 1-10):
...
Be HONEST and SPECIFIC. Do not inflate scores. A score of 5 means "mediocre". 7 means "good enough". 9-10 means "excellent". Garbled or hallucinated text should ALWAYS score below 4 on text_accuracy. Character appearance contradicting the Story Bible roster should ALWAYS score below 4 on character_fidelity."""
    ...
    # ── Dynamic per-call user prompt (scene-specific context) ─────────────
    CRITIQUE_USER = """NARRATIVE CONTEXT:
...
Critique the attached image against the scoring rubric and respond in the JSON schema specified in your system instructions."""
    ...
    def critique(self, image_path: str, scene_title: str, scene_description: str,
        ...
        # ── post-grill F1.2: scene context goes in USER prompt, rubric in SYSTEM ──
        user_prompt = self.CRITIQUE_USER.format(
        ...
        logger.info(f"Art Director reviewing: {Path(image_path).name} (scene {scene_number}/{total_scenes})")

        try:
            parsed = self.llm.critique_image(
                prompt=user_prompt,
                image_path=image_path,
                system_instruction=self.CRITIQUE_SYSTEM,  # ← rubric cached by API
                scene_num=scene_number or None,
            )
