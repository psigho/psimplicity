...
- If the script is abstract/philosophical, illustrate the CONCRETE EXAMPLES it uses, not the abstract concepts"""
    ...
    def _build_scenes(self, scenes_data: list) -> List[Scene]:
        ...
        for i, s in enumerate(scenes_data):
            ...
            # Coerce LLM-supplied scene_number to int — it later flows into
            # image filenames; never let a raw model string reach a Path component.
            try:
                _scene_no = int(s.get("scene_number", i + 1))
            except (TypeError, ValueError):
                _scene_no = i + 1

            scene = Scene(
                scene_number=_scene_no,
