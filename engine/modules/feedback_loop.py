...
class PromptSurgeon:
    ...
    def apply_fixes(
        ...
        # ── Skip-surgeon optimization (post-grill F1.6 / §4) ────────────────
        # If every detected error has a deterministic negative-inject only, the LLM rewrite
        # adds nothing — apply the negatives and a positive_additions append, no LLM call.
        all_neg_only = all(
            (ERROR_RULES.get(e.type, {}).get("fix_strategy") == "negative_inject")
            for e in errors
        )
        if all_neg_only and not positive_additions:
            logger.info("🔧 Prompt Surgeon skipped — deterministic negative-inject only")
            enhanced_prompt = prompt
        else:
            logger.info(f"🔧 Prompt Surgeon applying {len(errors)} fix(es) via LLM rewrite (role=surgeon)...")
            try:
                enhanced_prompt = llm_gateway.generate_text(
                    prompt=user_prompt,
                    system_instruction=system_instruction,
                    role="surgeon",  # ← post-grill F1.3: cheap Flash, not premium Pro
                    temp=0.7
                )
                # Sanity-check the rewrite — bail back to deterministic if obviously broken
                if not enhanced_prompt or len(enhanced_prompt) < max(20, len(prompt) // 4):
                    raise ValueError(f"Surgeon returned suspiciously short rewrite ({len(enhanced_prompt or '')} chars)")
                if len(enhanced_prompt) > len(prompt) * 3 + 500:
                    logger.warning(f"Surgeon rewrite is {len(enhanced_prompt)} chars (orig {len(prompt)}) — accepting but flagging")
            except Exception as e:
                # post-grill F1.6: deterministic fallback, NOT a silent re-send of the failing prompt.
                # Append the explicit positive reinforcements + a per-error directive block.
                logger.error(f"Prompt Surgeon LLM rewrite failed: {e}. Applying deterministic fallback fixes.")
                directive_lines = []
                for err in errors:
                    if err.detail:
                        directive_lines.append(f"- {err.type}: {err.detail}")
                directives_block = "\n".join(directive_lines[:6])
                pos_block = ". ".join(p for p in positive_additions if p)[:600]
                enhanced_prompt = prompt
                if pos_block:
                    enhanced_prompt = f"{enhanced_prompt}. {pos_block}"
                if directives_block:
