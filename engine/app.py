...
from concurrent.futures import ThreadPoolExecutor  # post-/goal F6.1: threaded pipeline

# Streamlit script-run-ctx propagation for background thread → session_state access
# (post-/goal F6.1)
try:
    from streamlit.runtime.scriptrunner import add_script_run_ctx, get_script_run_ctx
    _HAS_SCRIPTRUNNER_CTX = True
except Exception:
    _HAS_SCRIPTRUNNER_CTX = False

# Load .env file
load_dotenv()
from modules.orchestrator import Orchestrator
from modules.envutil import resolve_env_text

# Configure logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

def _resolve_env(val: str) -> str:
    """Expand ${VAR} patterns to actual env values (shared resolver — envutil)."""
    return resolve_env_text(val)
...
<style>
    ...
    /* ──── Polish pass (additive, rerun-safe — no entrance animations) ── */
    html { scroll-behavior: smooth; }
    /* Keyboard-accessibility focus ring on all interactive controls */
    ...
    /* Unify control easing so hovers feel consistent across the app */
    .stButton > button,
    .stDownloadButton > button,
    .image-card,
    [data-testid="stExpander"] summary {
    ...
    /* Tame oversized emoji on metric containers / gradient legibility on small screens */
    @media (max-width: 640px) {
        .ancap-logo { font-size: 2rem; }
        .main-title { font-size: 1.8rem; }
        .stMainBlockContainer { padding: 18px 18px !important; margin: 6px 8px 8px 8px; }
    }
</style>
...
<b style="color:#d4af37;">Personality:</b> {bb.get('brand_personality', 'N/A')[:100]}
    ...
    with st.expander("🎛 Engine Selector", expanded=True):
        ...
        # post-/goal fix 2026-06-01: expose the picked model id so the runtime
        # injection (gpt_image, etc.) maps the dropdown choice to the actual model.
        selected_image_model = img_list[img_idx].get("id", "")
        ...
        if st.session_state.run_folder:
            ...
            if st.button("📂 Open Output Folder", key="open_folder_btn"):
                ...
                if abs_folder.exists():
                    # List-form Popen: no shell, no quote-parsing — a folder name
                    # containing a stray '"' can never break out into a command.
                    subprocess.Popen(["explorer", str(abs_folder)])
...
# ── Cost estimate & budget ceiling (audit P0) ───────────────────────────────
with st.expander("💸 Cost estimate & budget", expanded=False):
    try:
        from modules.runtime_inject import load_config as _load_cfg_est
        from modules.costing import estimate_run_cost as _estimate
        _cfg_est = _load_cfg_est(config_path)
        _img_per_scene = int(_cfg_est.get("pipeline", {}).get("images_per_scene", 1) or 1)
        _scenes_known = target_scenes if (target_scenes and target_scenes > 0) else 0
        _scenes_for_est = _scenes_known or 6  # typical 3–8 min script ≈ ~6 scenes
        _est = _estimate(
            _cfg_est, num_scenes=_scenes_for_est, images_per_scene=_img_per_scene,
            max_retries=max_retries, image_id=globals().get("selected_image_model", ""),
            config_key=selected_image_config_key,
            parser_model=selected_parser_model, critic_model=selected_critic_model,
        )
        _basis = (f"{_scenes_known} scenes" if _scenes_known
                  else "~6 scenes (auto-detect — set Target Scenes for a precise figure)")
        _engine_lbl = globals().get("selected_image_model", "") or selected_image_config_key
        st.markdown(
            f"**Estimate** for {_basis} · `{_engine_lbl}` @ "
            f"${_est['image_unit_usd']:.2f}/image, max retries {max_retries}:"
        )
        _c1, _c2, _c3 = st.columns(3)
        _c1.metric("Typical", f"${_est['total_typical']:.2f}")
        _c2.metric("Worst case", f"${_est['total_max']:.2f}",
                   help=f"Every key visual burns all retries → {_est['images_max']} images")
        _c3.metric("Images typ/max", f"{_est['images_typical']}/{_est['images_max']}")
        st.caption("Images dominate; LLM token counts are assumed. Live cost shows during the run.")
    except Exception as _est_err:
        st.caption(f"Estimate unavailable: {_est_err}")

    if "run_budget_usd" not in st.session_state:
        st.session_state["run_budget_usd"] = 0.0
    st.number_input(
        "Hard budget ceiling (USD · 0 = no cap)",
        min_value=0.0, step=0.50, format="%.2f", key="run_budget_usd",
        help="The run aborts gracefully once total spend (images + LLM) crosses this.",
    )
...
with col_stop:
    def _stop_pipeline():
        # post-/goal F6.1: signal the worker thread to stop on its next callback tick
        ev = st.session_state.get("pipeline_stop_event")
        if ev is not None:
            try:
                ev.set()
            except Exception:
...
_EASTER_EGGS = [
    ...
    # ── Round 7: Raju (Darkshama / Alcestis) ──
    "💀 Echoing Darkshama's defeat against a fresh warlock — Scene {n} fears nothing…",
    "🔮 Alcestis is channeling a rez on Scene {n}… don't move…",
    "⚡ Darkshama dropped Heroism but still wiped on Scene {n}…",
    "🐺 Raju's ghost wolf is sprinting to deliver Scene {n}…",
    "🌊 Darkshama chain-healed Scene {n} back to full HP…",
    "☠️ Alcestis got feared into the next zone — Scene {n} loading from there…",
    # ── Round 8: Cluumsy — Brother in Chaos, Goodlife Warlock ──
    "🔥 Cluumsy just Chaos Bolted Scene {n} — crit for 10/10…",
    "😈 Cluumsy's felguard is tanking the render while he Life Taps…",
    "💜 Brother in Chaos Cluumsy dropped a Soulwell for Scene {n}…",
    "🌀 Cluumsy feared the pixels into formation — Scene {n} is ready…",
    "🪦 Cluumsy Drain Soul'd the last frame — Scene {n} is yours…",
    "☄️ Cluumsy Rain of Fire'd the canvas — Scene {n} emerging from the flames…",
    # ── Round 9: Terkus — The Moonkin ──
    "🌙 Terkus is in Moonkin Form — Scene {n} has +5% spell crit…",
    "🦉 Terkus just Starfired Scene {n} from orbit…",
    "🌿 Terkus popped Treants to speed up Scene {n}…",
    "☀️ Terkus hit Solar Eclipse — Scene {n} is radiating…",
    "🌕 Terkus Starfalled across the canvas — Scene {n} is glowing…",
    "💫 The big boomkin Terkus moonfire-spammed Scene {n} into existence…",
    # ── Round 10: Omi — The GOAT, Best Friend, Born Winner ──
    "👑 Omi walked in and Scene {n} rendered itself out of respect…",
    "🏆 Everything Omi touches turns to gold — Scene {n} included…",
    "🐐 The GOAT Omi doesn't generate images — images generate themselves for him…",
    "💯 Omi doesn't need AI — AI needs Omi. Scene {n} agrees…",
    "🥇 Omi came first again. Scene {n} never stood a chance…",
    "⭐ Scene {n} just looked at Omi and said 'I'll render myself, king'…",
    "🎖️ Omi's win rate is 100% — Scene {n} already knows the outcome…",
]
...
# ── post-/goal F6.1: Fragment-based progress display ────────────────────────
#  WHY: prior implementation ran orchestrator.run() synchronously and
#       Streamlit raised StopException on ANY widget click → script restarted
#       from line 0 → user saw "the interface just resets". Fix is a background
#       thread + auto-refreshing fragment that survives reruns.
@st.fragment(run_every=0.75)
def render_pipeline_progress_fragment():
    """Auto-refreshing progress display — survives widget clicks during run."""
    if not st.session_state.get("pipeline_running"):
        return

    p = st.session_state.get("pipeline_progress", {})
    pct = float(p.get("pct", 0.0) or 0.0)
    msg = p.get("msg", "Working...")

    st.markdown("### ⏳ Generating…")
    st.progress(max(0.0, min(1.0, pct)), text=msg)

    egg_msg = p.get("egg_msg", "")
    if egg_msg:
        st.markdown(
            f'<div style="text-align:center; color:#8b949e; font-size:0.85rem; '
            f'padding:4px 0; font-style:italic;">{egg_msg}</div>',
            unsafe_allow_html=True,
        )

    # Live cost/token ticker (post-/goal F6.6)
    cost_usd = p.get("cost_usd", 0.0)
    if cost_usd > 0:
        st.markdown(
            f'<div style="text-align:right; color:#00d4aa; font-size:0.78rem; '
            f'font-family: \'JetBrains Mono\', monospace; padding:2px 0;">'
            f'💸 ${cost_usd:.4f} · {p.get("calls", 0)} calls · '
    ...
    future = st.session_state.get("pipeline_future")
    if future is None:
        return
    if not future.done():
        return
    ...
    try:
        ...
        ts = report.timestamp
        pname = st.session_state.get("pipeline_project_name", "")
        safe_name = re.sub(r'[^\w\-]', '_', pname) if pname else ""
        folder_prefix = f"run_{safe_name}_{ts}" if safe_name else f"run_{ts}"
        out_folder = st.session_state.get("pipeline_out_folder", "output")
        st.session_state.run_folder = str(Path(out_folder) / folder_prefix)
    ...
    st.session_state.pipeline_future_started = False
    try:
        ex = st.session_state.get("pipeline_executor")
        if ex:
...
if st.session_state.pipeline_running and script_text and selected_preset:
    ...
    # Shared loader + runtime image-engine injection (see modules/runtime_inject.py).
    # Replaces the verbatim copy that used to live in both this path and the redo path.
    from modules.runtime_inject import load_config, inject_image_provider_config
    ...
    selected_image_config_key, _inject_err = inject_image_provider_config(
        config, selected_image_config_key, globals().get("selected_image_model", ""),
    )
    if _inject_err:
        st.error(_inject_err)
        st.session_state.pipeline_running = False
        st.stop()

    # ── post-/goal F6.1: Spawn the pipeline in a background thread on FIRST entry ──
    #  Subsequent script reruns (caused by widget clicks) will see future_started=True
    #  and skip directly to the fragment, which polls the future.
    status_placeholder.empty()

    if not st.session_state.get("pipeline_future_started"):
        # Shared mutable dict — closure-captured by thread, read by fragment.
        # We mutate it in place from the worker (no session_state writes from thread).
        progress_dict = {
            "pct": 0.0, "msg": "Starting pipeline...", "image": None,
            "egg_msg": "", "egg_ts": 0.0,
            "cost_usd": 0.0, "calls": 0, "tokens_in": 0, "tokens_out": 0,
            "stopped": False,
        }
        st.session_state.pipeline_progress = progress_dict

        # Stop signal — main thread sets it; worker callback polls it.
        ...
        try:
            orchestrator = Orchestrator(
                ...
                image_model_id=globals().get("selected_image_model", ""),
            )
        except Exception as e:
            st.error(f"Failed to init pipeline: {e}")
            import traceback
            st.code(traceback.format_exc())
            st.session_state.pipeline_running = False
            st.session_state.pipeline_future_started = False
        ...
        # Live cost ticker (now wired): the orchestrator pushes a running snapshot
        # into progress_dict after every LLM call AND every generated image, so the
        # fragment's $ / calls / token line updates in real time.
        def _cost_sink(snap):
            progress_dict["cost_usd"] = snap.get("usd", 0.0)
            progress_dict["calls"] = snap.get("calls", 0)
            progress_dict["tokens_in"] = snap.get("tokens_in", 0)
            progress_dict["tokens_out"] = snap.get("tokens_out", 0)
            progress_dict["images"] = snap.get("images", 0)

        def _progress_cb(scene_num, total, status, image_path):
            if stop_event.is_set():
                raise RuntimeError("Pipeline stopped by user.")
            progress_dict["pct"] = (scene_num / total) if total and total > 0 else 0.0
            progress_dict["msg"] = status
            if image_path:
                progress_dict["image"] = image_path
            now = time.time()
            if now - progress_dict.get("egg_ts", 0) > 4.0:
                progress_dict["egg_msg"] = _pick_easter_egg(scene_num, total)
                progress_dict["egg_ts"] = now

        # Capture locals needed for the thread closure (don't read session_state from worker)
        _script_text = script_text
        _selected_preset = selected_preset
        _target_scenes = target_scenes
        _project_name = project_name if project_name else None
        _brand_bible = st.session_state.get("brand_bible_data")
        _budget_usd = float(st.session_state.get("run_budget_usd", 0.0) or 0.0)

        def _run_pipeline():
            return orchestrator.run(
                script=_script_text,
                style_preset_path=_selected_preset,
                progress_callback=_progress_cb,
                target_scenes=_target_scenes,
                project_name=_project_name,
                brand_bible_data=_brand_bible,
                budget_usd=_budget_usd,
                cost_sink=_cost_sink,
            )

        executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="psim-pipeline")
        ctx = get_script_run_ctx() if _HAS_SCRIPTRUNNER_CTX else None
        future = executor.submit(_run_pipeline)
        if ctx is not None and _HAS_SCRIPTRUNNER_CTX:
            for _t in list(executor._threads):
                add_script_run_ctx(_t, ctx)

        st.session_state.pipeline_future = future
        st.session_state.pipeline_executor = executor
        st.session_state.pipeline_future_started = True
        st.session_state.pipeline_project_name = project_name or ""
        st.session_state.pipeline_out_folder = config["output"]["folder"]

    # Render the auto-refreshing progress fragment. From this point, widget clicks
    # in the rest of the page do NOT reset the running pipeline — only the
    # fragment re-runs every 0.75s, not the whole script.
    render_pipeline_progress_fragment()
...
if st.session_state.redo_scene:
    ...
    try:
        # Same shared loader + injection as the launch path. (Also fixes a latent
        # gap: this path never injected the gpt_image block, so a GPT-Image redo
        # used to fail — inject_image_provider_config now handles it.)
        from modules.runtime_inject import load_config, inject_image_provider_config
        config = load_config(config_path)
        config["pipeline"]["pass_threshold"] = pass_threshold
        config["pipeline"]["max_retries"] = max_retries

        _redo_image_key, _redo_inject_err = inject_image_provider_config(
            config, selected_image_config_key, globals().get("selected_image_model", ""),
        )
        if _redo_inject_err:
            raise RuntimeError(_redo_inject_err)
