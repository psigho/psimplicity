...
# ── Custom CSS ───────────────────────────────────────────────────────────────
from views.styles import get_custom_css, inject_js
...
<b style="color:#d4af37;">Mood:</b> {', '.join(preset_data.get('mood_keywords', []))}
    ...
    st.divider()
    with st.expander("📥 Process Client Revisions", expanded=False):
        st.caption("Pull feedback directly from the client QC report.")
        if st.button("Fetch Pending Revisions", use_container_width=True):
            supabase_url = os.environ.get("SUPABASE_URL", "")
            supabase_service_key = os.environ.get("SUPABASE_SERVICE_KEY", "")
            if not supabase_url or not supabase_service_key:
                st.error("Supabase SUPABASE_SERVICE_KEY missing in .env")
            else:
                with st.spinner("Connecting to Supabase securely..."):
                    try:
                        import requests
                        headers = {
                        ...
                        res = requests.get(
                            f"{supabase_url}/rest/v1/qc_feedback?status=eq.reject&select=*&order=created_at.desc",
                            headers=headers
                        )
                        if res.status_code == 200:
                            rejections = res.json()
                            if not rejections:
                                st.success("No pending revisions found!")
                            else:
                                st.session_state.pending_revisions = rejections
                                st.success(f"Found {len(rejections)} rejected scenes.")
                        else:
                            st.error(f"Failed to fetch: {res.text}")
                    except Exception as e:
                        st.error(f"Error connecting: {e}")
                        
        if st.session_state.get("pending_revisions"):
            st.markdown("### Pending Rejections")
            # Group by session
            grouped = {}
            for rev in st.session_state.pending_revisions:
                sid = rev['session_id']
                if sid not in grouped: grouped[sid] = []
                grouped[sid].append(rev)
                
            for sid, revs in grouped.items():
                st.markdown(f"**Session:** `{sid}`")
                for r in revs:
                    st.markdown(f"- **Scene {r['scene_id']}**: {r['notes']}")
                if st.button(f"Load & Queue {len(revs)} Revisions", key=f"load_{sid}"):
                    # Select this session in the main UI
                    for i, s in enumerate(sessions):
                        if s["folder"] == sid:
                            st.session_state.session_selector = i
                            st.session_state.new_project_mode = False
                            
                            # Automatically queue up these scenes for redo
                            st.session_state.redo_queue = revs
                            
                            # Mark them as processed in Supabase (basic REST PATCH)
                            supabase_url = os.environ.get("SUPABASE_URL", "")
                            supabase_service_key = os.environ.get("SUPABASE_SERVICE_KEY", "")
                            headers = {
                            ...
                            try:
                                import requests
                                for r in revs:
                                    requests.patch(f"{supabase_url}/rest/v1/qc_feedback?id=eq.{r['id']}", headers=headers, json={"status": "processed"})
                            except: pass
                            
                            st.rerun()
                            break
                    else:
                        st.error("Session folder not found locally. Ensure it wasn't deleted.")
...
def _pick_easter_egg(scene_num=0, total=0):
    """Return a non-repeating easter egg with scene number substituted.

    Uses a shuffled-deck pattern: every message is shown once before any
    can repeat, making back-to-back duplicates impossible.
    """
    # Session-state deck so it survives Streamlit re-runs during a pipeline
    if "easter_egg_deck" not in st.session_state or not st.session_state.easter_egg_deck:
        deck = list(_EASTER_EGGS)
        random.shuffle(deck)
        st.session_state.easter_egg_deck = deck
    msg = st.session_state.easter_egg_deck.pop()
...
if st.session_state.pipeline_running and script_text and selected_preset:
    ...
    try:
        ...
        try:
            from modules.qc_reporter import generate_interactive_report
            qc_results = {}
            for res in report.results:
                qc_results[res.scene_id if hasattr(res, 'scene_id') else str(res.scene_number)] = {
                    'status': 'success' if res.passed else 'failed',
                    'prompt': res.final_prompt,
                    'path': res.visual_results[0].file_path if res.visual_results else '',
            ...
            if supabase_url and supabase_key:
                qc_path = generate_interactive_report(
                    output_dir=st.session_state.run_folder,
                    original_script=orig_script,
                    generation_results=qc_results,
                    supabase_url=supabase_url,
                    supabase_key=supabase_key
                )
                logging.info(f"Generated QC report at {qc_path}")
                st.session_state.qc_report_path = qc_path
        except Exception as e:
            logging.error(f"Failed to generate QC Report: {e}")
...
# ── Auto-Redo Queue Handler ──────────────────────────────────────────────────
if not st.session_state.get("redo_scene") and st.session_state.get("redo_queue"):
    next_rev = st.session_state.redo_queue.pop(0)
    try:
        scene_num = int(next_rev["scene_id"])
    except ValueError:
        scene_num = 1
    
    st.session_state.redo_scene = {
        "scene_number": scene_num,
        "guidance": f"CLIENT FEEDBACK: {next_rev.get('notes', '')}",
        "custom_prompt": None
    }
    st.rerun()

# ── Redo Handler ─────────────────────────────────────────────────────────────
if st.session_state.get("redo_scene"):
